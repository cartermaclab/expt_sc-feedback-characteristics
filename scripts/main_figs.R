...
color_expt1 <- c("#5e81ac","#88c0d0", "#bf616a", "#d08770")
color_expt2 <- c("#a3be8c", "#b48ead")
...
expt1_9blocks_g <- dplyr::bind_rows(
  expt1_performance_acq_g,
  expt1_performance_prt_g
)

expt1_9blocks_g <- expt1_9blocks_g %>%
  dplyr::mutate(
...
# Fig 2a - Spatial total error (E) for all phases
fig2a <- ggplot2::ggplot(
  ...
  scale_y_continuous(name = "Spatial E (deg)",
                     limits = c(0, 30),
                     breaks = seq(0, 30, 5)) +
  scale_linetype_manual(values = c(1, 3, 5, 7),
                        labels = c("Choice+Error",
                                   "Choice+Graded",
                                   "Yoked+Error",
                                   "Yoked+Graded")) +
  scale_color_manual(values = color_expt1,
                     labels = c("Choice+Error",
                                "Choice+Graded",
                                "Yoked+Error",
                                "Yoked+Graded")) +
  scale_fill_manual(values = color_expt1,
                    labels = c("Choice+Error",
                               "Choice+Graded",
                               "Yoked+Error",
                               "Yoked+Graded")) +
...
# Fig 2b - Timing total error (E) for all phases
...
# Fig 2c - Perceived autonomy
fig2c <- ggplot2::ggplot(
  expt1_qaire_pa_g,
  aes(x = time_id,
      y = mean_rating,
      color = group_id,
  ...
  geom_point(data = expt1_qaire_pa_p,
             aes(x = time_id,
                 y = score,
                 color = group_id,
                 shape = group_id),
...
# Fig 2d - Perceived competence
fig2d <- ggplot2::ggplot(
  expt1_qaire_pc_g,
  aes(x = time_id,
      y = mean_rating,
      color = group_id,
  ...
  geom_pointrange(aes(ymin = ci_low,
                  ...
                  show.legend = FALSE,
                  position = position_dodge(0.9)) +
  geom_point(data = expt1_qaire_pc_p,
             aes(x = time_id,
                 y = score,
                 color = group_id,
                 shape = group_id),
...
# Fig 2e - Intrinsic motivation
fig2e <- ggplot2::ggplot(
  expt1_qaire_im_g,
  aes(x = time_id,
      y = mean_rating,
      color = group_id,
  ...
  geom_pointrange(aes(ymin = ci_low,
                  ...
                  show.legend = FALSE,
                  position = position_dodge(0.9)) +
  geom_point(data = expt1_qaire_im_p,
             aes(x = time_id,
                 y = score,
                 color = group_id,
                 shape = group_id),
...
# Create multipanel figure for Experiment 1

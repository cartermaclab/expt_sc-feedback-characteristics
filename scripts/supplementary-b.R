...
# and Carter
#
# Supplementary B (Error estimation) - Wrangle, analyses, and figures
#
# Authors:
#   Laura St. Germain
#   Brad McKay
...
expt1_performance_est_data <- readr::read_csv("data/expt1_performance-data.csv")
expt2_performance_est_data <- readr::read_csv("data/expt2_performance-data.csv")
...
# Create tibble with new column to identify those who were asked to estimate
# in pre-test and those who were not asked to compare acquisition performance
expt1_performance_est_tib <- expt1_performance_est_data %>%
...
# Make factors
expt1_performance_est_tib <- expt1_performance_est_tib %>%
  dplyr::mutate(
    group_id = forcats::as_factor(group_id),
    phase_id = forcats::as_factor(phase_id),
    block_id = forcats::as_factor(block_id),
    est_id = forcats::as_factor(est_id)
  )

# Tibble for retention and transfer data at the participant (p) level
expt1_performance_est_rt_p <- expt1_performance_est_tib %>%
  dplyr::filter(phase_id %in% c(3, 4)) %>%
  dplyr::group_by(id, group_id, block_id, est_id) %>%
  dplyr::summarize(
    n = n(),
...
# Tibble for acquisition at the participant (p) level
expt1_performance_est_acq_p <- expt1_performance_est_tib %>%
...
# Make factors
expt2_performance_est_tib <- expt2_performance_est_tib %>%
  dplyr::mutate(
    group_id = forcats::as_factor(group_id),
    phase_id = forcats::as_factor(phase_id),
    block_id = forcats::as_factor(block_id),
...
# Tibble for retention and transfer data at the participant (p) level
expt2_performance_est_rt_p <- expt2_performance_est_tib %>%
  dplyr::filter(phase_id %in% c(3, 4)) %>%
  dplyr::group_by(id, group_id, block_id, est_id) %>%
  dplyr::summarize(
    n = n(),
...
# Tibble for acquisition at the participant (p) level
expt2_performance_est_acq_p <- expt2_performance_est_tib %>%
  dplyr::filter(phase_id == 2) %>%
  dplyr::group_by(id, group_id, block_id, est_id) %>%
  dplyr::summarize(
    n = n(),
...
# ANALYSES: PERFORMANCE OF PRE-TEST ESTIMATORS VS NON-ESTIMATORS
#
# EXPERIMENT 1
#
# Spatial goal
#
# Acquisition
expt1_performance_est_acq_deg_lm <- afex::aov_ez(
  "id", "p_mean_e_deg", expt1_performance_est_acq_p,
  between = "est_id",
  within = "block_id"
)
expt1_performance_est_acq_deg_lm

# Retention and transfer
expt1_performance_est_rt_deg_lm <- afex::aov_ez(
  "id", "p_mean_e_deg", expt1_performance_est_rt_p,
  between = "est_id",
  within = "block_id"
)
expt1_performance_est_rt_deg_lm

# Timing goal
#
# Acquisition
expt1_performance_est_acq_ms_lm <- afex::aov_ez(
  "id", "p_mean_e_ms", expt1_performance_est_acq_p,
  between = "est_id",
  within = "block_id"
)
expt1_performance_est_acq_ms_lm

# Retention and transfer
expt1_performance_est_rt_ms_lm <- afex::aov_ez(
  "id", "p_mean_e_ms", expt1_performance_est_rt_p,
  between = "est_id",
  within = "block_id"
)
expt1_performance_est_rt_ms_lm
...
#
# Acquisition
expt2_performance_est_acq_deg_lm <- afex::aov_ez(
  "id", "p_mean_e_deg", expt2_performance_est_acq_p,
  between = "est_id",
  within = "block_id"
)
expt2_performance_est_acq_deg_lm

# Retention and transfer
expt2_performance_est_rt_deg_lm <- afex::aov_ez(
  "id", "p_mean_e_deg", expt2_performance_est_rt_p,
  between = "est_id",
  within = "block_id"
)
expt2_performance_est_rt_deg_lm

# Timing goal
#
# Acquisition
expt2_performance_est_acq_ms_lm <- afex::aov_ez(
  "id", "p_mean_e_ms", expt2_performance_est_acq_p,
  between = "est_id",
  within = "block_id"
)
expt2_performance_est_acq_ms_lm

# Retention and transfer
expt2_performance_est_rt_ms_lm <- afex::aov_ez(
  "id", "p_mean_e_ms", expt2_performance_est_rt_p,
  between = "est_id",
  within = "block_id"
)
expt2_performance_est_rt_ms_lm
